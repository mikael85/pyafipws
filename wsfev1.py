...
class WSFEv1(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        if "Errors" in ret:
            ...
            self.ErrCode = " ".join([str(error["Err"]["Code"])
                                    for error in errores])
            self.ErrMsg = "\n".join(self.Errores)
        if "Events" in ret:
            events = ret["Events"]
    ...
    def AgregarComprador(self, doc_tipo=80, doc_nro=0, porcentaje=100.00, **kwarg):
        ...
        comp = {"doc_tipo": doc_tipo,
                "doc_nro": doc_nro, "porcentaje": porcentaje}
    ...
    def CAESolicitar(self):
        ...
        if "FeCabResp" in result:
            ...
            self.FechaCbte = fedetresp.get(
                "CbteFch", "")  # .strftime("%Y/%m/%d")
    ...
    def CompConsultar(self, tipo_cbte, punto_vta, cbte_nro, reproceso=False):
        ...
        if "ResultGet" in result:
            ...
            cod_aut = (
                resultget["CodAutorizacion"] and str(
                    resultget["CodAutorizacion"]) or ""
            )  # 60423794871430L
...
def main():
    ...
    proxy = ""
    wrapper = ""  # "pycurl"
    # "aaa.crt" #"/home/reingart/.local/lib/python2.7/site-packages/certifi/cacert.pem" #
    ...
    if "--prueba" in sys.argv:
        ...
        for i in range(reg_x_req):
            ...
            assert wsfev1.EstablecerCampoFactura(
            ...
            if tipo_cbte in (2, 3, 7, 8, 12, 13, 202, 203, 208, 213):
                ...
                cuit = "20267565393"
                # obligatorio en Factura de Crédito Electrónica MiPyMEs (FCE):
                fecha_cbte = fecha if tipo_cbte in (
                    3, 202, 203, 208, 213) else None
        ...
        if "--consultar" in sys.argv:
            ...
            p_assert_eq(wsfev1.ObtenerCampoFactura(
            ...
            p_assert_eq(wsfev1.ObtenerCampoFactura(
                "tributos", 0, "importe"), 1)
            # comparar primer opcional
            if "--rg3668" in sys.argv:
                p_assert_eq(wsfev1.ObtenerCampoFactura(
                    "opcionales", 0, "valor"), "02")
    ...
    if "--solicitar-caea" in sys.argv:
        ...
        if DEBUG:
            print("Solicitando CAEA para periodo %s orden %s" %
                  (periodo, orden))
    ...
    if "--sinmovimiento-caea" in sys.argv:
        ...
        if DEBUG:
            print(
                "Informando Punto Venta %s CAEA %s SIN MOVIMIENTO" % (
                    punto_vta, caea)
            )
